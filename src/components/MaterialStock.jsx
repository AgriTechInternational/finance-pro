import React from 'react';

const fmtKg   = (n) => Number(n || 0).toLocaleString('en-EG', { maximumFractionDigits: 0 });
const fmtTons = (n) => Number(n || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const fmtBags = (n) => Number(n || 0).toLocaleString('en-US', { maximumFractionDigits: 0 });
const pctFmt  = (n) => (Number(n || 0) * 100).toFixed(1) + '%';

export default function MaterialStock({ data }) {
  // rawMaterialStock is in KG (from Material Inventory!J72)
  const rawMaterialStockKg  = data?.summary?.rawMaterialStock  || 0;
  const totalProducedTons   = data?.summary?.totalProduced     || 0;   // tons
  const totalProducedKg     = data?.summary?.totalProducedKg   || 0;   // kg
  const totalProducedBags   = data?.summary?.totalProducedBags || 0;

  const hasData = rawMaterialStockKg > 0;
  const remainingKg = Math.max(0, rawMaterialStockKg - totalProducedKg);

  return (
    <div className="page-fade" style={{ padding: 24 }}>
      <div className="page-header" style={{ marginBottom: 32 }}>
        <div className="page-title">Material Stock</div>
        <div className="page-sub">
          Raw material inventory (kg) — Source: Material Inventory tab · Cell J72
        </div>
      </div>

      {/* Primary KPI: Raw Material Available in KG */}
      <div style={{ marginBottom: 28 }}>
        <div
          style={{
            background: 'linear-gradient(135deg, rgba(59,130,246,0.12), rgba(37,99,235,0.06))',
            border: '1px solid rgba(59,130,246,0.3)',
            borderRadius: 20,
            padding: '40px 48px',
            display: 'flex',
            alignItems: 'center',
            gap: 40,
          }}
        >
          <div style={{ fontSize: 60 }}>📦</div>
          <div>
            <div
              style={{
                fontSize: 11,
                fontWeight: 900,
                textTransform: 'uppercase',
                letterSpacing: '0.2em',
                color: '#60a5fa',
                marginBottom: 10,
              }}
            >
              Raw Material Available (X)
            </div>
            <div
              style={{
                fontSize: 72,
                fontWeight: 900,
                color: 'var(--text1)',
                fontFamily: 'var(--mono)',
                lineHeight: 1,
              }}
            >
              {hasData ? fmtKg(rawMaterialStockKg) : '—'}
              <span style={{ fontSize: 28, marginLeft: 14, opacity: 0.5, fontWeight: 600 }}>
                kg
              </span>
            </div>
            {hasData && (
              <div style={{ fontSize: 13, color: 'var(--text3)', marginTop: 12 }}>
                ≈ {fmtTons(rawMaterialStockKg / 1000)} tons of unprocessed raw material in the factory.
              </div>
            )}
            {!hasData && (
              <div style={{ fontSize: 13, color: 'rgba(239,68,68,0.7)', marginTop: 12 }}>
                Cell J72 in Material Inventory is empty or not yet synced.
              </div>
            )}
          </div>
        </div>
      </div>

      {/* Secondary KPIs */}
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))',
          gap: 20,
        }}
      >
        {/* Finished Product */}
        <div
          className="card shadow-soft"
          style={{
            padding: 28,
            borderLeft: '4px solid var(--accent2)',
            background: 'linear-gradient(135deg, rgba(16,185,129,0.08), transparent)',
          }}
        >
          <div style={{ fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--accent2)', marginBottom: 10 }}>
            🏭 Finished Product (Y)
          </div>
          <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--text1)', fontFamily: 'var(--mono)', lineHeight: 1 }}>
            {fmtKg(totalProducedKg)}
            <span style={{ fontSize: 14, marginLeft: 8, opacity: 0.5 }}>kg</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 8 }}>
            {fmtBags(totalProducedBags)} bags · {fmtTons(totalProducedTons)} tons produced
          </div>
        </div>


        {/* Remaining Raw Material */}
        <div
          className="card shadow-soft"
          style={{
            padding: 28,
            borderLeft: '4px solid var(--accent)',
          }}
        >
          <div style={{ fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--accent)', marginBottom: 10 }}>
            🔄 Remaining Stock
          </div>
          <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--text1)', fontFamily: 'var(--mono)', lineHeight: 1 }}>
            {hasData ? fmtKg(remainingKg) : '—'}
            <span style={{ fontSize: 14, marginLeft: 8, opacity: 0.5 }}>kg</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 8 }}>
            Raw material not yet converted to finished goods
          </div>
        </div>
      </div>
    </div>
  );
}
